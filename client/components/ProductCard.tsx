  ...
  return (
    <Link href={`/product/${product._id}`} asChild>
        <TouchableOpacity className='w-[48%] mb-4 bg-white rounded-lg overflow-hidden'>
            <View className='relative h-56 w-full bg-grey-100'>
                ...
                <TouchableOpacity className='absolute  top-2 right-2 z-10 p-2 bg-white rounded-full shadow-sm' onPress={(e)=>{
                    ...
                    <Ionicons name={isLiked ? 'heart' : 'heart-outline'} size={26} color={isLiked ? COLORS.accent : COLORS.primary}/>
                </TouchableOpacity>

                {/* Is Featured */}

                {product.isFeatured && (
                    <View className='absolute top-2 left-2 bg-black px-2 py-1 rounded opacity-80'>
