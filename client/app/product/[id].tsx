...
export default function ProductDetails() {
    ...
    return (
        <View className='flex-1 bg-white'>
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
                ...
                <View className='relative h-[450px] bg-gray-100 mb-6'>
                    ...
                    <View className='absolute top-12 left-4 right-4 flex-row justify-between items-center z-10'>
                        <TouchableOpacity onPress={() => { router.back() }} className='w-10 h-10 rounded-full bg-white/80 items-center justify-center'>
                            <Ionicons name='arrow-back' size={24} color={COLORS.primary} />
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => { toggleWishlist(product) }} className='w-10 h-10 rounded-full bg-white/80 items-center justify-center'>
                    ...
                    <View className='absolute bottom-4 left-0 right-0 flex-row justify-center gap-2'>
                        {product.images?.map((_, index) => (
                            <View key={index} className={`h-2 rounded-full ${index === activeImageIndex} ? 'w-6 bg-primary' : 'w-2 bg-secondary' `} />
                        ))}
                    </View>
                ...
                <View className='px-5'>
                     {/* Title and Rating */}
                     <View className='text-2xl justify-between items-start mb-2 flex-row'>
                        <Text className='text-2xl font-bold ml-1 w-[260px]'>{product.name}</Text>
                        <View className='flex-row justify-between items-start mb-2 gap-1'>
                            <Ionicons name='star' size={20} color={'#FFD700'}/>
                            <Text className='text-sm font-bold mt-0.5'>4.6</Text>
                            <Text className='text-xs text-secondary ml-1 mt-1'>(85)</Text>
                        </View>
                     </View>
                    ...
                    {product.sizes && product.sizes.length > 0 && (
                        ...
                        <View className='flex-row gap-1.5 mb-6 flex-wrap'>
                            {product.sizes.map((size)=>(
                                <TouchableOpacity key={size} onPress={()=>{
                                    setSelectedSize(size)
                                }} className={`w-20 h-12 rounded-full items-center justify-center border ${selectedSize===size ? 'bg-primary border-primary' : 'bg-gray-100 border-gray-300'}`}>
            ...
            {/* Footer */}
            <View className='absolute bottom-0 left-0 flex-row right-0 p-5 bg-white border-t border-gray-200 '>
                <TouchableOpacity onPress={handleAddToCart} className='w-4/5 bg-primary py-4 rounded-full items-center justify-center flex-row shadow-lg ml-2'>
                    <Ionicons name='bag-outline' size={24} color='white'/>
                    <Text className='text-white font-bold text-base ml-2'>Add to Cart</Text>
                </TouchableOpacity>

                <TouchableOpacity onPress={()=>router.push("/(tabs)/cart")} className='w-1/5 rounded-full items-center justify-center flex-row shadow-lg relative'>
                    <Ionicons name='cart-outline' size={24} />
                    <View className='absolute top-2 right-5 size-4 z-10 bg-black rounded-full justify-center items-center'>
